...
void ResponseCurveComponent::drawBackgroundGrid(juce::Graphics& g)
{
    ...
    for (auto gDb : gain)
    {
        auto y = jmap(gDb, -6.f, 6.f, float(bottom), float(top));
...
void PathProducer::process(juce::Rectangle<float> fftBounds, double sampleRate)
{
    ...
    while (leftChannelFifo->getNumCompleteBuffersAvailable() > 0)
    {
        if (leftChannelFifo->getAudioBuffer(tempIncomingBuffer))
        {
            ...
            leftChannelFFTDataGenerator.produceFFTDataForRendering(monoBuffer, -6.f);
        }
    }
    ...
    while (leftChannelFFTDataGenerator.getNumAvailableFFTDataBlocks() > 0)
    {
        ...
        {
            pathProducer.generatePath(fftData, fftBounds, fftSize, binWidth, -6.f);
        }
    }
